...
No Docker, `GOOGLE_CREDENTIALS_FILE` e `GOOGLE_TOKEN_FILE` no `.env` devem apontar para arquivos existentes no host. O Compose monta esses arquivos no container como:
...
Não versione `.env`, credenciais Google ou tokens. `.dockerignore` e `.gitignore` excluem `.env`, `data/google/` e arquivos comuns de credenciais/tokens.
